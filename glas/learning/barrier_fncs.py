...
from glas.utils import torch_min_point_circle_rectangle, min_point_circle_rectangle
...
class Barrier_Fncs():
    ...
    def torch_get_grad_phi_dot(self, x, P, H):
        ...
        for j in range(self.get_num_neighbors(x) + self.get_num_obstacles(x)):
            ...
            f3dot = -torch.mul(torch.bmm(p_rel, v_rel).squeeze(2),
                               torch.mul(torch.pow(normp, -1), torch.pow(normp - self.param.r_agent, -2)))
    ...
    def torch_get_gradp2_phi(self, x, P, H):
        ...
        for j in range(self.get_num_neighbors(x) + self.get_num_obstacles(x)):
            ...
            if idx.nelement() == 1:
                ...
                grad_f2[idx] = torch.mul(f3[idx],
                                         torch.pow(torch.mul(normP[idx], torch.pow(f2[idx], 2)), -1)).unsqueeze(1)
    ...
    def torch_get_cf_si_2(self, x, pi, barrier_action, P, H):
        ...
        if not H.nelement() == 0:
            ...
            hidx = A2 > 0
            adaptive_scaling[idx] = torch.min( \
                torch.mul(A1[idx], torch.pow(A1[idx] + torch.abs(A2[idx]), -1)),
                torch.ones(1, device=self.device) - epsilon)
        # adaptive_scaling[idx] = torch.min(\
    ...
    def torch_get_relative_positions_and_safety_functions(self, x):
        ...
        for j in range(nn):
            ...
            P[:, curr_idx, :] = x[:, idx] * (
                        1 - self.param.r_agent * torch.pow(torch.norm(x[:, idx], p=2, dim=1).unsqueeze(1), -1))
            H[:, curr_idx] = (torch.norm(P[:, curr_idx, :], p=2, dim=1) - self.param.r_agent) / (
                        self.param.r_comm - self.param.r_agent)
            curr_idx += 1
    ...
    def numpy_fdbk_di(self, x, P, H):
        ...
        b = -1 * self.param.kv * (
                    v + self.param.kp * grad_phi) - self.param.kp * grad_phi_dot - self.param.kp * grad_phi
        return b
    ...
    def numpy_get_relative_positions_and_safety_functions(self, x):
        ...
        for j in range(nn):
            ...
            H[:, curr_idx] = (np.linalg.norm(P[:, curr_idx, :]) - self.param.r_agent) / (
                        self.param.r_obs_sense - self.param.r_agent)
            curr_idx += 1
        ...
        for j in range(no):
            ...
            closest_point = min_point_circle_rectangle(
                ...
                x[:, idx] + np.array([0.5, 0.5]))
            P[:, curr_idx, :] = closest_point
            H[:, curr_idx] = (np.linalg.norm(closest_point) - self.param.r_agent) / (
                        self.param.r_obs_sense - self.param.r_agent)
            curr_idx += 1
        return P, H
