...
# my package
from glas.learning.deepset import DeepSet, DeepSetObstacles
# from learning.empty_net import Empty_Net
from glas.learning.feedforward import FeedForward
        ...
        if self.param.rollout_batch_on:
            ...
            if len(grouping) < len(x):
                ...
                for key, idxs in grouping.items():
                    batch = torch.tensor([x[idx][0] for idx in idxs], device=device)
                    a = self(batch)
                    a = a.detach().numpy()
