...
def run_sim(param, env, controller, initial_state, device='cpu'):
    ...
    for step, time in enumerate(param.sim_times[:-1]):
        ...
        action = controller.policy(observation, device)
...
def sim(param, env, controllers, initial_state, visualize):
    ...
    for name, controller in controllers.items():
        ...
        if param.env_name in ['SingleIntegrator', 'SingleIntegratorVelSensing', 'DoubleIntegrator']:
            ...
            for agent in env.agents:
                ...
                plotter.plot_circle(result.states[1, env.agent_idx_to_state_idx(agent.i)],
                                    result.states[1, env.agent_idx_to_state_idx(agent.i) + 1], param.r_agent, fig=fig,
                                    ax=ax, color=color)
            ...
            if param.env_name in ['SingleIntegrator', 'SingleIntegratorVelSensing']:
                for step in np.arange(0, result.steps, 1000):
                    ...
                    plotter.plot_circle(result.states[step, env.agent_idx_to_state_idx(robot)],
                                        result.states[step, env.agent_idx_to_state_idx(robot) + 1], param.r_agent,
                    ...
                    robot_pos = result.states[step,
                                env.agent_idx_to_state_idx(robot):env.agent_idx_to_state_idx(robot) + 2]
        ...
        if name in ['PID_wRef', 'Ref']:
            ...
            for i in range(env.n):
                fig, ax = plotter.plot(times[1:result.steps + 1], ref_state[0:result.steps, i],
                                       title="ref " + env.states_name[i])

        # extract belief topology
        if name in ['IL'] and param.env_name == 'Consensus':
            for result in sim_results:
                if result.name == 'IL':
                    break
    ...
    save_path = os.path.join(save_dir, param.plots_fn)
    plotter.save_figs(save_path)
    # plotter.open_figs(save_path)
...
def run(args, param, env, controllers, initial_state=None):
