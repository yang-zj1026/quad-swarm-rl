...
DEFAULT_QUAD_REWARD_SHAPING_SINGLE = dict(
    quad_rewards=dict(
        pos=1.0, effort=0.05, spin=0.1, vel=0.0, crash_floor=5.0, orient=1.0, yaw=0.0,
    ),
)
...
DEFAULT_QUAD_REWARD_SHAPING['quad_rewards'].update(dict(
    quadcol_bin=0.0, quadsettle=0.0,
))
        ...
        for i, info in enumerate(infos_multi):
            ...
            if dones_multi[i]:
                ...
                if hasattr(self.env.unwrapped, 'scenario') and self.env.unwrapped.scenario:
                    ...
                    for rew_key in ['rew_pos', 'rewraw_pos', 'rew_crash_floor', 'rewraw_crash_floor']:
